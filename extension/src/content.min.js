...
(function () {
	...
	y.prototype.S = function (definition) {
		...
		// redraws the popup based on the current state of the application. this
		// consists of clearing and repopulating the recently viewed words tab,
		// populating the words dropdown and configuring their click handlers,
		// writing phonetics data (if present), write the currently selected
		// part of speech, populate the meanings area with word definitions,
		// resize the meanings container to ensure it's size matches its
		// content, and ensure the visibility of the popup
		function update() {
			// =================================================================
			// create a reference to the currently selected word object
			const selectedWordObj = igdx_wordData[igdx_selectedWord];
			// =================================================================

			// =================================================================
			// populate header tabs with the three most recently selected words,
			// or an empty string if there are <3 recently selected words
			...
			// =================================================================
			// populate dropdown with previously viewed words that aren't in the
			// three most recently viewed list
			const dropdownWords =
				Object.keys(igdx_wordData)
				.filter(word => igdx_wordCache.list.indexOf(word) === -1)
				.sort();
			igdx_tabHeader // set height of the dropdown based on no. of words
				.childNodes[3]
				.style
				.height = ((dropdownWords.length || 1) * 23.5) + "px";

			igdx_tabHeader // fill the dropdown element with word spans
				.childNodes[3]
				.innerHTML =
				 	dropdownWords
						.map(word => `
							<span class="igdx_dropdownWordOption">
								${word}
							</span>
						`)
						.join("")
			...
			if(dropdownWords.length === 0) {
				// place in empty list sentinel value, to prevent a zero-height
				// dropdown being drawn
				igdx_tabHeader
					.childNodes[3]
					.innerHTML = "<span class=\"igdx_dropdownWordOption\">No recently viewed words</span>";
			} else {
				// if the dropdown has words in it, attach click handlers
				for(let i = 0; i < dropdownWords.length; i++) {
					igdx_tabHeader
					.childNodes[3]
					.childNodes[i]
					.onclick = function() {
						// when a word is clicked, bump it to the front!
						bumpDefinition(dropdownWords[i]);
					}
				}
			}

			igdx_tabHeader // assign dropdown visibility based on current state
				.childNodes[3]
				.style
		...
		// function runs when either of the chevrons in the popup are clicked.
		// the chevron that was clicked is referenced through the chevronRef flag,
		// and is -1 if the left chevron was clicked, 1 otherwise.
		function chevronClickHandler(chevronRef) {
			// compute offset and load current word data
			const offset = (-1 * chevronRef) + (1 * !chevronRef);
			const selectedWordObj = igdx_wordData[igdx_selectedWord];

			// propose the next position and check its validity. if it's a valid
			// position, move to that position and redraw the popup.
		...
		function bumpDefinition(word) {
			// select between the word available via closure on the definition
			// object, or the word that is passed in to the function
			igdx_selectedWord = definition.word;
			if(word) igdx_selectedWord = word;

			// push the bumped word into the recent word cache and redraw the
			// popup
			igdx_wordCache.push(igdx_selectedWord);
			update();
		}

		// add a new word to the word databank and set its state. the prop
		// "partOfSpeech" denotes which index in the meanings array the user
		// currently has displayed on screen for that word
		...
		// initialize the main popup window. this includes creating all of the
		// necessary DOM elements, populating them with initial data, setting up
		// static click handlers, and inserting style rules
		function igdx_init() {
			...
			igdx_tabHeader.innerHTML = `
				<div style="width: calc(100% - 50px);">
					<span class="igdx_wordTab" style="color: #ff6e00; width: 33%;"></span>
					<span class="igdx_wordTab" style="width: 33%;"></span>
					<span class="igdx_wordTab" style="width: 33%;"></span>
				</div>
				...
				<div id="igdx_dropdown"></div>
			`.replace(/[\t\n]+/g, "");

			// add click handler for the grab bar to enable popup dragging
			igdx_tabHeader
				.childNodes[2]
			...
			// if the user is dragging the popup, compute the new position and
			// move the popup into place
			...
			// add click-end handler to the document so when the user releases
			// their mouse anywhere on the document, the dragging ends
			...
			// add click handlers to the recent word tabs that bump the
			// definition to the front when clicked
			for(let i = 0; i < 3; i++) {
				igdx_tabHeader
					.childNodes[0]
					.childNodes[i]
			...
			// attach click handler to the dropdown which toggles its visibility
			igdx_tabHeader
				.childNodes[1]
				.childNodes[0]
				.onclick = function() {
			...
			// attach click handler to the "x" which toggles visibility of the
			// popup
			igdx_tabHeader
				.childNodes[1]
				.childNodes[1]
			...
			// create document element that contains the definition data and
			// populate it with some blank starting elements
			...
			igdx_definitionContainer.innerHTML = `
				...
				<div id="igdx_meaningsContainer"></div>
			`.replace(/[\t\n]+/g, "");

			// attach click handlers to the part of speech chevrons (arrows)
			...
			// insert newly created elements into the primary popup box
			...
			// create a styles element to store our custom styling in
			...
			// styles for the grab bar where the user clicks and drags to move
			// the popup around. !important tags override styles from the tab
			// bar
			sheet.insertRule(`
				#igdx_grabBar {
			...
			// styles for the clickable words that appear in the dropdown
			sheet.insertRule(`
				.igdx_dropdownWordOption {
					width: 100% !important;
			...
			`);
			sheet.insertRule(`
				.igdx_dropdownWordOption:active {

				}
			`);

			// styles for the tab bar and the elements contained within
			sheet.insertRule(`
				#igdx_tabHeader div {
					display: flex;
					flex-direction: row;
			...
			sheet.insertRule(`
				#igdx_primaryBox {
					...
					border: 2px solid black;
					opacity: ${enableOpacity ? 0.3 : 1};
					z-index: 4294967295; /* 2^32 - 1; browsers typically use 32 bit integers for z indices */
				}
			`);
			if(enableOpacity) {
				// only add the opacity rule if it is enabled via a global flag
				sheet.insertRule(`
			...
			// styles for the chevrons (arrows)
			sheet.insertRule(`
			...
			// styles for the container where the word's part of speech is shown
			sheet.insertRule(`
				#igdx_posContainer {
					display: flex;
					flex-direction: row;
			...
			// styles for the area where word definitions are shown
			sheet.insertRule(`
			...
			// styles for the clickable phonetics items
			sheet.insertRule(`
			...
			// style for the additional words dropdown
			sheet.insertRule(`
				#igdx_dropdown {
					position: absolute;
