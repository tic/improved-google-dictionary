...
(function () {
	...
	*/
	var c = 0,
		...
		r = function () {
			var a;
			if (a = e.value.replace(/^\s+|\s+$/g, "")) g.textContent = "Searching...", f.style.display = "block", h.style.display = "none", k.style.display = "none", n.style.display = "none", d.disabled = !0, c++, chrome.runtime.sendMessage({
				type: "fetch_html",
				eventKey: c,
				query: a
			}, q)
		...
		q = function (a) {
			if (a.eventKey === c) {
				if (a.html) {
					n.innerHTML = a.html;
					var b = n.querySelectorAll(".audio");
					for (a = 0; a < b.length; a++) {
						var l = b[a],
							m = document.createElement("audio");
						m.preload = !0;
						"" === l.getAttribute("data-url") && (l.style.display = "none");
						m.src = l.getAttribute("data-url");
						l.addEventListener("click", m.play.bind(m), !1)
					}
					b = n.querySelectorAll(".nym-link");
					for (a = 0; a < b.length; a++) b[a].addEventListener("click", function () {
						e.value = this.title ? this.title : this.innerHTML;
						r();
						return !1
					}, !1);
					b = n.querySelectorAll('a:not([class="nym-link"])');
					for (a = 0; a < b.length; a++) p(b[a]);
					f.style.display = "none";
					n.style.display = "block"
				} else g.textContent =
					"No definition found.", f.style.display = "block", h.href = "https://www.google.com/search?q=" + a.sanitizedQuery, h.innerHTML = 'Search the web for "' + a.sanitizedQuery + '" &raquo;', h.style.display = "block";
				d.disabled = !1
			}
		};
	...
	chrome.tabs.query({
		...
		a.length && (console.assert(1 === a.length), chrome.tabs.sendMessage(a[0].id, {
			...
			b && b.selection && (e.value = b.selection, r())
		}))
	});
})();
