...
(function () {
	...
	y.prototype.S = function (definition) {
		...
		// runs when the user clicks the "x" on the popup
		// reset all variables and delete #igdx_primaryBox from the DOM
		function igdx_destroy() {
		...
		function igdx_init() {
			...
			//
			// `);
			// styles for the part of speech container
			sheet.insertRule(`
				.igdx_posContainer {
					position: relative;
					width: 130px;
			...
			`);
			// styles for the span containing the part of speech
			sheet.insertRule(`
				.igdx_posContainer #igdx_selectedPartOfSpeech {
					position: relative;
					top: 3px;
				}
			`);
			// styles for the pronunciation button / speaker icon
			sheet.insertRule(`
				.igdx_phoneticSpeaker {
					cursor: pointer;
					width: 25px;
				}
			`);
			// styles for the meaning left chevron
			sheet.insertRule(`
				.igdx_meaningLeft {
					position: absolute;
					left: 0;
			...
			`);
			// styles for the meaning right chevron
			sheet.insertRule(`
				.igdx_meaningRight {
					position: absolute;
					right: 0;
		...
		function meaningArrowClickHandler(isLeft) {
			console.log(isLeft ? "left" : "right", this);
		}
		...
		function addDefinition() {
			igdx_wordData[definition.word] = {
				...
				state: {
					meaning: 0,
					definitions: definition.meanings.reduce(
						(acc, cur, i) => {
							acc[i] = {
								current: 0,
								maximum: cur.definitions.length - 1
							};
							return acc;
						},
						{}
			...
			var cardHtml = `
				<div class="igdx_header">
					<span class="igdx_word">${definition.word}</span>
					<div class="igdx_phonetics">
						${definition.phonetics.map(phonetic => `
							<div class="igdx_phonetic" style="position: relative;">
								${
									phonetic.audio ?
										`<span class="igdx_phoneticSpeaker" onclick="(new Audio('${phonetic.audio}')).play(); return false">
											&#128364;
										</span>
				...
				<div class="igdx_posContainer">
					<span class="igdx_meaningLeft">
						&#9204;
					</span>
					<span id="igdx_selectedPartOfSpeech" style="width: 45px;">
						${definition.meanings[0].partOfSpeech}
						</span>
					<span class="igdx_meaningRight">
						&#9205;
					</span>
				</div>
				<div>
					<span id="igdx_meaning">
			...
			element.setAttribute("class", "igdx_definition");
			element.setAttribute("word", definition.word);
			element.innerHTML = cardHtml;
			document.body.appendChild(element);

			element.getElementsByClassName("igdx_meaningLeft")[0].onclick = function() {
				meaningArrowClickHandler.bind(this)(true);
			}
			element.getElementsByClassName("igdx_meaningRight")[0].onclick = function() {
				meaningArrowClickHandler.bind(this)(false);
			}
		}
		...
		if(!igdx_wordData[definition.word])
			...
			bumpDefinition();
					...
					// // if the word has not been defined,
					// if(!igdx_words.includes(definition.word)) {
					// 	var cardHtml = `
					// 		<div class="igdx_header">
					// 			<span class="igdx_word">${definition.word}</span>
					// 			<div class="igdx_phonetics">
					...
					// 						${
					// 							phonetic.audio ?
					// 								`<span class="igdx_phoneticSpeaker" onclick="(new Audio('${phonetic.audio}')).play(); return false">&#128364;</span><span style="margin-left: 6px;">${phonetic.text}</span>`
					// 								: `<span style="position: relative; left: 25px;">${phonetic.text}</span>`
					// 						}
					// 					</div>
					// 				`.replace(/[\n\t]+/g, "")).join("")}
					// 			</div>
					// 		</div><div id="data" style="display: none;">${JSON.stringify(definition)}</div><div id="state" style="display: none;">${JSON.stringify({
					// 			meaning: 0,
					// 			maxMeaning: definition.meanings.length - 1,
					// 			definition: definition.meanings.reduce(
					// 				(acc, cur, i) => {
					// 					acc[i] = {
					// 						current: 0,
					// 						maximum: cur.definitions.length - 1
					...
					// 				{}
					// 			)
					// 		})}</div><div class="igdx_posContainer"><span class="igdx_meaningLeft">&#9204;</span><span id="igdx_selectedPartOfSpeech" style="width: 45px;">${definition.meanings[0].partOfSpeech}</span><span class="igdx_meaningRight">&#9205;</span></div><div><span id="igdx_meaning">${definition.meanings[0].definitions[0].definition}</span></div>
					// 		<div>
					// 		</div>
					// 	`;
					// 	var element = document.createElement("div");
					// 	element.setAttribute("class", "igdx_definition");
					// 	element.setAttribute("word", definition.word);
					// 	element.style.top = words.length * 150;
					// 	element.innerHTML = cardHtml;
					//
					// 	// Create left button click handler
					// 	element.getElementsByClassName("igdx_meaningLeft")[0].onclick = function() {
					// 		// Load current state
					// 		const stateNode = this.parentNode.previousSibling;
					// 		let state = JSON.parse(stateNode.innerHTML);
					//
					// 		if(state.meaning > 0) {
					// 			const definitionNodes = stateNode.nextSibling.nextSibling.childNodes;
					// 			const dataNode = stateNode.previousSibling;
					// 			const data = JSON.parse(dataNode.innerHTML);
					//
					// 			state.meaning--;
					// 			const { definition: defn, example } = data.meanings[state.meaning].definitions[state.definition[state.meaning].current];
					// 			definitionNodes[0].innerHTML = defn;
					//
					// 			const partOfSpeechNode = stateNode.nextSibling.childNodes[1];
					// 			partOfSpeechNode.innerHTML = data.meanings[state.meaning].partOfSpeech;
					// 		}
					//
					// 		// Update current state
					// 		stateNode.innerHTML = JSON.stringify(state);
					// 	}
					//
					// 	// Create right button click handler
					// 	element.getElementsByClassName("igdx_meaningRight")[0].onclick = function() {
					// 		// Load current state
					// 		const stateNode = this.parentNode.previousSibling;
					// 		let state = JSON.parse(stateNode.innerHTML);
					//
					// 		if(state.meaning < state.maxMeaning) {
					// 			const definitionNodes = stateNode.nextSibling.nextSibling.childNodes;
					// 			const dataNode = stateNode.previousSibling;
					// 			const data = JSON.parse(dataNode.innerHTML);
					//
					...
					// 	}
					//
					// 	// Write element into DOM
					// 	gdc.appendChild(element);
					//
					// 	// Add word to the list of displayed words
					// 	igdx_words.push(definition.word);
					// } else {
					// 	// the user wants to define a word that is already defined on screen
					// 	// for now, ignore this. eventually, it might be nice to highlight
					// 	// 	the existing definition or something.
					// }
